...
# Create a frame for the sidebar on the left side (List of words)
sidebar_frame = tk.Frame(root, width=200, bg="white")
sidebar_frame.grid(row=0, column=0, sticky="ns")

# Create a frame for the central puzzle grid (Grid Frame)
...
info_panel = tk.Frame(root, width=300, bg="white")
...
for i in range(len(crossword)):
    ...
    for j in range(len(crossword[i])):
        ...
        btn.bind("<ButtonPress-1>", lambda event, i=i, j=j: handle_button_event(event, selected_buttons, button_grid, correct_words, word_buttons, info_panel, scientist_images))
        btn.bind("<B1-Motion>", lambda event, i=i, j=j: handle_button_event(event, selected_buttons, button_grid, correct_words, word_buttons, info_panel, scientist_images))
        btn.bind("<ButtonRelease-1>", lambda event, i=i, j=j: handle_button_event(event, selected_buttons, button_grid, correct_words, word_buttons, info_panel, scientist_images))
        row.append(btn)
...
def resize_grid(event):
    ...
    for i in range(len(button_grid)):
        for j in range(len(button_grid[i])):
            btn = button_grid[i][j]
            btn.config(width=button_size//8, height=button_size//15, font=("Arial", button_size//2))
